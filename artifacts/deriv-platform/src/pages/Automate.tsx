...
const Automate = () => {
  ...
  const netUnitRef = useRef(0); // for D'Alembert
  const lastResultWasWinRef = useRef<boolean | null>(null); // for Oscar's Grind
  ...
  const nextStakeAfterLoss = (current: number): number => {
    ...
    // oscars_grind: keep flat after a loss
  ...
  const nextStakeAfterWin = (current: number): number => {
    ...
    // martingale resets to base stake after a win
    return parseFloat(baseStake);
  };
